...
which_method <- "TIRA"
AD_or_ALL <- "ALL" # AD= symptoms after positivity, ALL=symptoms before and after positivity
...
if (AD_or_ALL == "AD") {
  df_ADNI_TIRA <- read.csv(here("data_final","converter_df_TIRA_ADNI.csv"))
  df_ADNI_TIRA$ID <- as.factor(df_ADNI_TIRA$ID)
  
  df_KADRC_TIRA <- read.csv(here("data_final","converter_df_TIRA_KADRC.csv"))
  df_KADRC_TIRA$ID <- as.factor(df_KADRC_TIRA$ID)
  
  df_ADNI_SILA <- read.csv(here("data_final","converter_df_SILA_ADNI.csv"))
  df_ADNI_SILA$ID <- as.factor(df_ADNI_SILA$ID)
  
  df_KADRC_SILA <- read.csv(here("data_final","converter_df_SILA_KADRC.csv"))
  df_KADRC_SILA$ID <- as.factor(df_KADRC_SILA$ID)
} else if (AD_or_ALL == "ALL") {
  df_ADNI_TIRA <- read.csv(here("data_final","converter_df_sens_TIRA_ADNI.csv"))
  df_ADNI_TIRA$ID <- as.factor(df_ADNI_TIRA$ID)
  
  df_KADRC_TIRA <- read.csv(here("data_final","converter_df_sens_TIRA_KADRC.csv"))
  df_KADRC_TIRA$ID <- as.factor(df_KADRC_TIRA$ID)
  
  df_ADNI_SILA <- read.csv(here("data_final","converter_df_sens_SILA_ADNI.csv"))
  df_ADNI_SILA$ID <- as.factor(df_ADNI_SILA$ID)
  
  df_KADRC_SILA <- read.csv(here("data_final","converter_df_sens_SILA_KADRC.csv"))
  df_KADRC_SILA$ID <- as.factor(df_KADRC_SILA$ID)
} else {
  stop("Error: Choose correct dataset")
}

symps_ADNI_TIRA <- read.csv(here("data_final","df_symptoms_ADNI_FINAL.csv"))
symps_ADNI_TIRA$ID <- as.factor(symps_ADNI_TIRA$ID)

symps_KADRC_TIRA <- read.csv(here("data_final","df_symptoms_KADRC_FINAL.csv"))
symps_KADRC_TIRA$ID <- as.factor(symps_KADRC_TIRA$ID)

symps_ADNI_SILA <- read.csv(here("data_final","df_symptoms_ADNI_FINAL.csv")) %>% 
  filter(!is.na(EXAMDATE))
symps_ADNI_SILA$ID <- as.factor(symps_ADNI_SILA$ID)

symps_KADRC_SILA <- read.csv(here("data_final","df_symptoms_KADRC_FINAL.csv"))

...
which_dataset <- "ADNI" # Choose from "KADRC" or "ADNI"
which_method <- "SILA" # Choose from "TIRA" or "SILA"

which_outcome <- "CDR_NEW"
filter_CDR_0_at_bl <- FALSE

sila_coef <- read.csv(here("results","linear_models_SILA_CDR_DX_Imp_FINAL.csv"))
tira_coef <- read.csv(here("results","linear_models_TIRA_CDR_DX_Imp_FINAL.csv"))
...
if (which_dataset == "ADNI" & which_method == "TIRA") {
  df <- read.csv(here("results","LongitudinalData_ADNI_dataset_TIRA_method_FINAL.csv"))
  symps <- read.csv(here("data_final","df_symptoms_ADNI_FINAL.csv")) %>% 
  ...
  df <- read.csv(here("results","LongitudinalData_KADRC_dataset_TIRA_method_FINAL.csv"))
  symps <- read.csv(here("data_final","df_symptoms_KADRC_FINAL.csv"))
  result_tmp <- df_linear[df_linear$Dataset == which_dataset & df_linear$Model == which_method, c("intercept", "slope")]
  model_intercept <- result_tmp$intercept
  model_slope <- result_tmp$slope
} else if (which_dataset == "ADNI" & which_method == "SILA") {
  df <- read.csv(here("results","LongitudinalData_ADNI_dataset_SILA_method_FINAL.csv"))
  symps <- read.csv(here("data_final","df_symptoms_ADNI_FINAL.csv")) %>% 
    mutate(CDR_DX = CDR_DX_Imp)
  result_tmp <- df_linear[df_linear$Dataset == which_dataset & df_linear$Model == which_method, c("intercept", "slope")]
  model_intercept <- result_tmp$intercept
  ...
  df <- read.csv(here("results","LongitudinalData_KADRC_dataset_SILA_method_FINAL.csv"))
  symps <- read.csv(here("data_final","df_symptoms_KADRC_FINAL.csv"))
  result_tmp <- df_linear[df_linear$Dataset == which_dataset & df_linear$Model == which_method, c("intercept", "slope")]
  model_intercept <- result_tmp$intercept
  model_slope <- result_tmp$slope

...
if (which_dataset == "ADNI") {
  df0 <- read.csv(here("data_raw","merged_data_CLEAN_FUtime_ge_1.csv"), header = TRUE)
} else if (which_dataset == "KADRC") {
  df0 <- read.csv(here("data_raw","KADRC_data_NEW_2025_04_03_FUtime_ge_1.csv"), header = TRUE)
} else {

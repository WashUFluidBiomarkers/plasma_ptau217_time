...
which_method <- "TIRA" # Choose between "TIRA" or "SILA"

if (which_method == "TIRA") {
  clock_adni <- read.csv(here("results","df_clock_ADNI_FINAL.csv"))
  df_adni <- read.csv(here("results","TrajPlotData_ADNI_dataset_TIRA_method_FINAL.csv"))
  
  clock_kadrc <- read.csv(here("results","df_clock_KADRC_FINAL.csv"))
  df_kadrc <- read.csv(here("results","TrajPlotData_KADRC_dataset_TIRA_method_FINAL.csv"))
  
} else if (which_method == "SILA") {
  clock_adni <- read.csv(here("results_SILA","SILA_clock_ADNI_2025-04-27.csv")) %>% 
    mutate(clock_time = adtime)
  df_adni <- read.csv(here("results","TrajPlotData_ADNI_dataset_SILA_method_FINAL.csv")) %>% 
    arrange(ID, EXAMDATE) %>% 
    group_by(ID) %>%
    mutate(years_since_onset = AGE - est_onset_age) %>% 
    ungroup()
  
  clock_kadrc <- read.csv(here("results_SILA","SILA_clock_KADRC_2025-04-27.csv")) %>% 
    mutate(clock_time = adtime)
  df_kadrc <- read.csv(here("results","TrajPlotData_KADRC_dataset_SILA_method_FINAL.csv"))  %>% 
    arrange(ID, EXAMDATE) %>% 
    group_by(ID) %>%
    mutate(years_since_onset = AGE - est_onset_age) %>% 
    ungroup()
